...
class Newsletter
{
    ...
    // fonction d'enregistremant d'un email
    public function save_email()
    {
        ...
        if (isset($_POST['easy_newsletter_email']) && !empty($_POST['easy_newsletter_email']))
        {
            ...
            global $wpdb;
            $row = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}easy_newsletter WHERE email = '$email'");
            if(is_null($row)) {
                $wpdb->insert("{$wpdb->prefix}easy_newsletter", array('email' => $email));
            }
        } 
    }
}
